...
def recommend(anime_title, top_n=5):
    ...
    return df.iloc[anime_indices][['Title', 'Genre', 'Rating', 'Link']]
...
if st.button("推荐"):
    ...
    if results.empty:
        ...
        for _, row in results.iterrows():
            st.markdown(f"**[{row['Title']}]({row['Link']})** | ⭐ {row['Rating']}<br>类型: {row['Genre']}", unsafe_allow_html=True)

...
import pandas as pd
from function.item_based_filtering_function import item_based_filtering_recommend

st.title("Item-Based Collaborative Filtering")

session_state_format()
st.session_state.where_page = "item_based_page"
session_check_where()
...
if recommend_clicked:
    ...
    st.markdown("---")
    st.session_state.item_based_results, st.session_state.item_based_anime_select_details = item_based_filtering_recommend(
        df_animes,
        df_reviews,
        selected_anime
    )
...
if (
    ...
    details = st.session_state.item_based_anime_select_details.iloc[0]
    ...
    if details['title'] == selected_anime:
        ...
        title = item_based_anime_select_details.iloc[0]['title']
        genre = item_based_anime_select_details.iloc[0]['genre']
        rating = item_based_anime_select_details.iloc[0]['score']
        synopsis = item_based_anime_select_details.iloc[0]['synopsis']
        link = item_based_anime_select_details.iloc[0]['link']
        ...
        current_result_page = st.session_state.item_based_result_page
        start = current_result_page * per_page
        end = start + per_page
        
        title_recommender_results = st.session_state.item_based_results
